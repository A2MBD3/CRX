...
(function () {
  ...
  function mapMember(raw) {
    ...
    const level = raw.level || raw.Level || raw.role || raw.Role || raw.type || raw.Type || "member";
  ...
  /* ── LEVEL-BASED CARD ACCENT COLORS ────────────────────── */
  ...
  async function render() {
    ...
    try {
      ...
      departments.forEach(d => {
        filterHTML += `<button class="filter-chip" data-filter="dept-${escapeHTML(d)}">${escapeHTML(d)}</button>`;
      });
      ...
      function renderCards(filter = "all") {
        ...
        if (filter === "all") {
          ...
          filtered = members.filter(m => m.department === dept);
        } else {
          filtered = members;
        }
